...
void CForm::Save_Object(const CString& strFilePath)
{
	...
	for (const auto& Terrain : m_pMapToolView->m_listTerrain)
	{
		...
		WriteFile(hFile, &NowTerrainInfo->X, sizeof(int), &dwByte, nullptr);
		WriteFile(hFile, &NowTerrainInfo->Z, sizeof(int), &dwByte, nullptr);
		WriteFile(hFile, &NowTerrainInfo->Interval, sizeof(int), &dwByte, nullptr);
		WriteFile(hFile, &NowTerrainInfo->Detail, sizeof(int), &dwByte, nullptr);

		const auto& vecTextureInfo = static_cast<CToolGameObject*>(Terrain)->Get_vecTextureInfo();
		//폴더명 길이
		int TerrainTextureFolderLength = 0;
		//파일명 길이
		int TerrainTexturePathLength = 0;
		for (const TEXTUREINFO& TextureInfo : vecTextureInfo)
		{
			TerrainTextureFolderLength = TextureInfo.strTextureFolder.length() + 1;
			WriteFile(hFile, &TerrainTextureFolderLength, sizeof(int), &dwByte, nullptr);
			//lstrcpy(strFolder, m_pMapToolView->m_tTextureFolder);
			WriteFile(hFile, TextureInfo.strTextureFolder.c_str(), sizeof(TCHAR) * TerrainTextureFolderLength, &dwByte, nullptr);

			TerrainTexturePathLength = TextureInfo.strTextureName.length() + 1;
			WriteFile(hFile, &TerrainTexturePathLength, sizeof(int), &dwByte, nullptr);
			WriteFile(hFile, TextureInfo.strTextureName.c_str(), sizeof(TCHAR) * TerrainTexturePathLength, &dwByte, nullptr);
		}
		CString strObjectName;
		static_cast<CToolGameObject*>(Terrain)->Get_ObjectName(strObjectName);
		WriteFile(hFile, strObjectName.GetString(), sizeof(TCHAR) * strObjectName.GetLength() + 1, &dwByte, nullptr);

		CString strTypeName;
		static_cast<CToolGameObject*>(Terrain)->Get_TypeName(strTypeName);
		WriteFile(hFile, strTypeName.GetString(), sizeof(TCHAR) * strTypeName.GetLength() + 1, &dwByte, nullptr);

		const _vec3& vec3Scale = Terrain->getTransform()->getScale();
		WriteFile(hFile, &vec3Scale, sizeof(_vec3), &dwByte, nullptr);
		const _matrix& pTransform = Terrain->getTransform()->getRotateAxis();
		WriteFile(hFile, &pTransform, sizeof(_matrix), &dwByte, nullptr);
		const _vec3& vec3Position = Terrain->getTransform()->getPos();
		WriteFile(hFile, &vec3Position, sizeof(_vec3), &dwByte, nullptr);

		HTREEITEM hTreeParent;
		static_cast<CToolGameObject*>(Terrain)->Get_TreeName(&hTreeParent, TREENAME::PARENT);
		std::wstring strParent = m_Tree_Object.GetItemText(hTreeParent).GetString();
		int iParentLength = strParent.length() + 1;
		// 만약 문자열의 길이가 1이면은 그 계층은 없는것임 로드할때 주의할것
		WriteFile(hFile, &iParentLength, sizeof(int), &dwByte, nullptr);
		WriteFile(hFile, strParent.c_str(), sizeof(TCHAR) * iParentLength, &dwByte, nullptr);

		HTREEITEM hTreeNow;
		static_cast<CToolGameObject*>(Terrain)->Get_TreeName(&hTreeNow, TREENAME::NOW);
		std::wstring strNow = m_Tree_Object.GetItemText(hTreeNow).GetString();
		int iNowLength = strNow.length() + 1;
		WriteFile(hFile, &iNowLength, sizeof(int), &dwByte, nullptr);
		WriteFile(hFile, strNow.c_str(), sizeof(TCHAR) * iNowLength, &dwByte, nullptr);

		HTREEITEM hTreeChild;
		static_cast<CToolGameObject*>(Terrain)->Get_TreeName(&hTreeChild, TREENAME::CHILD);
		std::wstring strChild = m_Tree_Object.GetItemText(hTreeChild).GetString();
		int iChildLength = strChild.length() + 1;
		WriteFile(hFile, &iChildLength, sizeof(int), &dwByte, nullptr);
		WriteFile(hFile, strChild.c_str(), sizeof(TCHAR) * iChildLength, &dwByte, nullptr);
...
void CForm::Load_Object(const CString& strFilePath)
{
